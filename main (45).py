...
def init_db():
    ...
    cur.execute("""
        ...
        ALTER TABLE users ADD COLUMN IF NOT EXISTS curator_name TEXT DEFAULT '';
        ALTER TABLE users ADD COLUMN IF NOT EXISTS lesson_days TEXT DEFAULT '';
        ALTER TABLE users ADD COLUMN IF NOT EXISTS lesson_time TEXT DEFAULT '';
...
def get_all_users():
    conn = get_conn(); cur = conn.cursor(cursor_factory=RealDictCursor)
    cur.execute("SELECT *, (SELECT MAX(day) FROM user_completions uc WHERE uc.user_id=users.user_id) AS app_day FROM users ORDER BY joined_at DESC")
    rows = cur.fetchall(); cur.close(); conn.close(); return rows

def get_broadcast_users(audience=None):
    conn = get_conn(); cur = conn.cursor(cursor_factory=RealDictCursor)
    q = "SELECT * FROM users WHERE COALESCE(bot_blocked, FALSE)=FALSE"
    if audience == "a1":
        q += " AND allowed_sections LIKE '%a1%'"
    elif audience == "b1":
        q += " AND allowed_sections LIKE '%b1%'"
...
def get_profile_stats(uid):
    ...
    cur.execute("SELECT curator_name, lesson_days, lesson_time FROM users WHERE user_id=%s", (uid,))
    _sch = cur.fetchone()
    cur.close(); conn.close()
    return {"completed_days": completed_days, "learned_words": learned_words,
            ...
            "curator_name": (_sch[0] if _sch else "") or "",
            "lesson_days": (_sch[1] if _sch else "") or "",
            "lesson_time": (_sch[2] if _sch else "") or ""}
...
def send_lesson_reminders():
    """Dars kuni bo'lgan o'quvchilarga eslatma (Toshkent, ertalab)."""
    try:
        tz = pytz.timezone("Asia/Tashkent")
        today = datetime.now(tz).weekday() + 1
        conn = get_conn(); cur = conn.cursor()
        cur.execute("SELECT user_id, curator_name, lesson_time, lesson_days FROM users "
                    "WHERE lesson_days IS NOT NULL AND lesson_days <> '' AND COALESCE(bot_blocked,FALSE)=FALSE")
        rows = cur.fetchall(); cur.close(); conn.close()
        import urllib.request, urllib.parse
        for uid, curator, ltime, ldays in rows:
            days = [x for x in (ldays or "").split(",") if x]
            if str(today) not in days:
                continue
            msg = "\U0001F4DA Bugun dars kuningiz!\n\n"
            if ltime:
                msg += "Soat " + ltime + " da "
            if curator:
                msg += "ustoz " + curator + " bilan "
            msg += "individual darsingiz bor. Tayyor bo'ling! \U0001F4AA"
            try:
...
def api_tts():
    ...
    ckey = "tts/" + hashlib.md5(("alena|wav16|" + text).encode("utf-8")).hexdigest() + ".wav"
    if r2_configured():
        try:
            ...
            pass
    import re as _re2, struct as _st
    _parts = _re2.split(r"(?<=[.!?\n])\s+", text)
    _chunks = []; _cur = ""
    for _p in _parts:
        if len(_cur) + len(_p) + 1 > 400 and _cur:
    ...
    try:
        for _ch in _chunks:
            body = urllib.parse.urlencode({"text": _ch, "lang": "ru-RU", "voice": "alena",
                                           "folderId": folder, "format": "lpcm", "sampleRateHertz": "16000"}).encode()
    ...
    _sr = 16000; _n = len(pcm)
    audio = (b"RIFF" + _st.pack("<I", 36 + _n) + b"WAVE" + b"fmt " +
             _st.pack("<IHHIIHH", 16, 1, 1, _sr, _sr * 2, 2, 16) + b"data" + _st.pack("<I", _n) + pcm)
    if r2_configured():
        try:
            get_r2_client().put_object(Bucket=R2_BUCKET, Key=ckey, Body=audio, ContentType="audio/wav")
        except Exception:
            pass
...
def admin_ai_shadowing():
    ...
    try:
        ...
        if not level:
            return {"ok": False, "error": "level yo'q"}, 400
        txt = (get_section_pdf_text(level, day, "shadowing") or get_section_pdf_text(level, day, "speaking") or get_section_pdf_text(level, day, "audio"))
        if not txt:
...
def admin():
    ...
    def _user_row(u):
        ...
        return (
            ...
            "data-status='%s' data-label='%s' data-new='%s' data-joined='%s' data-streak='%s' data-appday='%s' data-search='%s %s %s'>"
        ...
        ) % (uid, nm, ",".join(granted), mode, until, st, lab, isnew, jsort, streak, appday,
...
<div class="sidebar" id="sidebar">
  ...
  <button class="nav-item" data-owner="1" data-panel="jurnal" onclick="nav(this);loadJurnal();">📋 Jurnal</button>
...
<div class="main">
  ...
  <div class="panel" id="p-users">
    ...
    <div id="userModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:60;align-items:center;justify-content:center;padding:16px;">
      <div style="background:var(--card);border-radius:16px;padding:20px;max-width:420px;width:100%;max-height:90vh;overflow:auto;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-bottom:16px;">
          ...
          <div class="udcell"><div class="udk">Ilovada</div><div class="udv" id="ud-appday">—</div></div>
    ...
    <div id="accModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:60;align-items:center;justify-content:center;padding:16px;">
      <div style="background:var(--card);border:1px solid var(--border);border-radius:18px;max-width:430px;width:100%;max-height:92vh;overflow-y:auto;-webkit-overflow-scrolling:touch;">
        ...
        <div style="padding:16px 20px;">
          ...
          <div id="acc-labels" style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:18px;">
            ...
            <button type="button" class="lblbtn" data-lbl="kurator" onclick="pickLabel(this)">🎓 Kurator bilan</button>
          </div>
          <div style="border:1px solid var(--border);border-radius:12px;padding:12px;margin-bottom:14px;">
            <div class="muted" style="font-size:11px;font-weight:600;margin-bottom:8px;">🎓 USTOZ BILAN DARS (ixtiyoriy)</div>
            <input id="acc-curator" placeholder="Ustoz ismi" style="width:100%;padding:9px;border:1px solid var(--border);border-radius:8px;margin-bottom:8px;font-family:inherit;font-size:13px;background:var(--card);color:var(--text);">
            <div style="font-size:11px;color:#8894ad;margin-bottom:5px;">Dars kunlari:</div>
            <div id="acc-days" style="display:flex;gap:5px;margin-bottom:8px;flex-wrap:wrap;">
              <label style="font-size:12px;display:flex;align-items:center;gap:3px;cursor:pointer;"><input type="checkbox" value="1"> Du</label>
              <label style="font-size:12px;display:flex;align-items:center;gap:3px;cursor:pointer;"><input type="checkbox" value="2"> Se</label>
              <label style="font-size:12px;display:flex;align-items:center;gap:3px;cursor:pointer;"><input type="checkbox" value="3"> Cho</label>
              <label style="font-size:12px;display:flex;align-items:center;gap:3px;cursor:pointer;"><input type="checkbox" value="4"> Pa</label>
              <label style="font-size:12px;display:flex;align-items:center;gap:3px;cursor:pointer;"><input type="checkbox" value="5"> Ju</label>
              <label style="font-size:12px;display:flex;align-items:center;gap:3px;cursor:pointer;"><input type="checkbox" value="6"> Sh</label>
              <label style="font-size:12px;display:flex;align-items:center;gap:3px;cursor:pointer;"><input type="checkbox" value="7"> Ya</label>
            </div>
            <div style="font-size:11px;color:#8894ad;margin-bottom:5px;">Soat:</div>
            <input id="acc-time" type="time" style="padding:9px;border:1px solid var(--border);border-radius:8px;font-family:inherit;font-size:13px;background:var(--card);color:var(--text);">
          </div>
  ...
  <div class="panel" id="p-jurnal">
    <h1>📋 O'quvchilar jurnali</h1>
    <div id="jurnal-list-view">
      <div class="hint" style="margin-bottom:10px;">«🎓 Kurator bilan» belgilangan o'quvchilar. Dars qo'shish uchun tanlang. (Dostup'da «Kurator bilan» belgilang.)</div>
      <div id="jurnal-list"><div class="muted" style="padding:10px">Yuklanmoqda...</div></div>
    </div>
    <div id="jurnal-detail-view" style="display:none;">
      <button class="backbtn" onclick="backToJurnal()">← Ro'yxat</button>
      <div style="font-weight:800;font-size:17px;margin:8px 0 4px;" id="jurnal-title">—</div>
      <div style="background:var(--card);border:1px solid var(--border);border-radius:12px;padding:14px;margin:12px 0 16px;">
        <div style="font-weight:700;margin-bottom:10px;">➕ Yangi dars</div>
        <input type="date" id="jl-date" style="width:100%;padding:10px;border:1px solid var(--border);border-radius:9px;margin-bottom:8px;font-family:inherit;">
        <div style="display:flex;gap:8px;margin-bottom:8px;">
          <label style="flex:1;display:flex;align-items:center;gap:6px;font-size:14px;cursor:pointer;"><input type="radio" name="jl-st" value="done" checked> ✓ Bo'ldi</label>
          <label style="flex:1;display:flex;align-items:center;gap:6px;font-size:14px;cursor:pointer;"><input type="radio" name="jl-st" value="missed"> ✗ Bo'lmadi</label>
        </div>
        <input type="text" id="jl-topic" placeholder="Mavzu (masalan: Род)" style="width:100%;padding:10px;border:1px solid var(--border);border-radius:9px;margin-bottom:8px;font-family:inherit;">
        <textarea id="jl-note" placeholder="Izoh (ixtiyoriy)" style="width:100%;min-height:60px;padding:10px;border:1px solid var(--border);border-radius:9px;margin-bottom:8px;font-family:inherit;resize:vertical;"></textarea>
        <button onclick="addLesson()" style="width:100%;padding:11px;border:none;border-radius:10px;background:#25c274;color:#fff;font-weight:700;cursor:pointer;">💾 Saqlash</button>
      </div>
      <div style="font-weight:700;margin-bottom:8px;">Darslar tarixi</div>
      <div id="jurnal-lessons"></div>
    </div>
  </div>
  <div class="panel" id="p-broadcast">
    ...
    <div class="section">
      ...
      <div style="margin:12px 0;">
        <div class="hint" style="font-weight:700;margin-bottom:5px;">👥 Kimga yuborilsin?</div>
        <select id="bc_audience" style="width:100%;border:1px solid var(--border);border-radius:10px;padding:10px;font-family:inherit;font-size:14px;">
          <option value="">🌍 Hammaga</option>
          <option value="a1">📘 Faqat A1 (Asosiy kurs)</option>
          <option value="b1">📗 Faqat B1 (Ilg'or kurs)</option>
        </select>
      </div>
...
function saveAccess(){
  ...
  var _days=[];document.querySelectorAll('#acc-days input:checked').forEach(function(c){_days.push(c.value);});
  var _cur=(document.getElementById('acc-curator')||{}).value||'';
  var _tm=(document.getElementById('acc-time')||{}).value||'';
  fetch('/admin/user-access',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({user_id:ACC_UID,sections:secs,mode:mode,until:until,notify:(document.getElementById('acc-notify')||{}).checked,curator_name:_cur,lesson_days:_days.join(','),lesson_time:_tm})}).then(function(r){if(r.ok)location.reload();else alert('Xato');});}
...
function openUserDetail(el){
  ...
  var _ad=document.getElementById('ud-appday');if(_ad)_ad.textContent=(d.appday&&d.appday!=='0')?(d.appday+'-kun'):'Boshlamagan';
...
var JUR_UID=null,JUR_NAMES={};
function jEsc(s){return (s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');}
function loadJurnal(){
  fetch('/admin/jurnal-students').then(function(r){return r.json();}).then(function(j){
    var el=document.getElementById('jurnal-list');if(!el)return;
    if(!j.ok||!j.students||!j.students.length){el.innerHTML="<div class='muted' style='padding:10px'>«Kurator bilan» belgilangan o'quvchi yo'q. Avval Dostup'da belgilang.</div>";return;}
    JUR_NAMES={};
    el.innerHTML=j.students.map(function(s){
      JUR_NAMES[s.user_id]=s.name;
      var appd=s.app_day?(s.app_day+'-kun'):'Boshlamagan';
      return "<div class='chrow' onclick='openJurnalStudent("+s.user_id+")'><div class='chav'>"+jEsc((s.name||'?').charAt(0).toUpperCase())+"</div><div class='chmid'><div class='chname'>"+jEsc(s.name)+"</div><div class='chlast'>📱 Ilovada: "+appd+" · "+s.count+" dars</div></div></div>";
    }).join('');
  });
}
function openJurnalStudent(uid){
  JUR_UID=uid;
  document.getElementById('jurnal-list-view').style.display='none';
  document.getElementById('jurnal-detail-view').style.display='';
  document.getElementById('jurnal-title').textContent=JUR_NAMES[uid]||('ID '+uid);
  loadJurnalLessons();
}
function loadJurnalLessons(){
  fetch('/admin/jurnal-lessons?uid='+encodeURIComponent(JUR_UID)).then(function(r){return r.json();}).then(function(j){
    var el=document.getElementById('jurnal-lessons');if(!el)return;
    if(!j.ok||!j.lessons||!j.lessons.length){el.innerHTML="<div class='muted' style='padding:8px'>Hali dars yo'q</div>";return;}
    el.innerHTML=j.lessons.map(function(l){
      var badge=(l.status==='done')?"<span style='background:#e7f7ef;color:#0e8a73;font-weight:700;font-size:11px;padding:3px 8px;border-radius:6px;'>✓ Bo'ldi</span>":"<span style='background:#fdeaec;color:#c0392b;font-weight:700;font-size:11px;padding:3px 8px;border-radius:6px;'>✗ Bo'lmadi</span>";
      var top=l.topic?("<div style='font-size:13px;margin-top:5px;'>📖 "+jEsc(l.topic)+"</div>"):"";
      var nt=l.note?("<div style='font-size:12px;color:#8894ad;margin-top:3px;'>"+jEsc(l.note)+"</div>"):"";
      return "<div style='background:var(--card);border:1px solid var(--border);border-radius:10px;padding:11px;margin-bottom:8px;'><div style='display:flex;justify-content:space-between;align-items:center;'><b style='font-size:13px;'>"+jEsc(l.date||'—')+"</b>"+badge+"</div>"+top+nt+"<div style='text-align:right;margin-top:4px;'><span onclick='delLesson("+l.id+")' style='color:#c0392b;font-size:11px;cursor:pointer;'>🗑 o'chirish</span></div></div>";
    }).join('');
  });
}
function addLesson(){
  var date=document.getElementById('jl-date').value;
  var stEl=document.querySelector('input[name=jl-st]:checked');
  var st=stEl?stEl.value:'done';
  var topic=document.getElementById('jl-topic').value.trim();
  var note=document.getElementById('jl-note').value.trim();
  fetch('/admin/lesson-add',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({user_id:JUR_UID,date:date,status:st,topic:topic,note:note})}).then(function(r){return r.json();}).then(function(j){
    if(j&&j.ok){document.getElementById('jl-topic').value='';document.getElementById('jl-note').value='';loadJurnalLessons();}
    else alert((j&&j.error)||'Xato');
  }).catch(function(){alert('Xato');});
}
function delLesson(id){
  if(!confirm('Bu dars ochirilsinmi?'))return;
  fetch('/admin/lesson-del',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id})}).then(function(r){return r.json();}).then(function(j){if(j&&j.ok)loadJurnalLessons();else alert('Xato');});
}
function backToJurnal(){document.getElementById('jurnal-detail-view').style.display='none';document.getElementById('jurnal-list-view').style.display='';loadJurnal();}
var vSec='talaffuz';
...
async function sendBroadcast(){
  if(bcType==='text'){
    ...
    var aud=(document.getElementById('bc_audience')||{}).value||'';
    var audTxt=(aud==='a1')?'A1 uchun':((aud==='b1')?'B1 uchun':'hammaga');
    if(!sched&&!confirm(audTxt+' hozir yuborilsinmi?'))return;
    var r=await fetch('/admin/broadcast',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:t,buttons:btns,send_at:send_at,audience:aud})});
    if(r.ok){var j=await r.json();if(j.scheduled){alert('Rejalashtirildi ✓');location.reload();return;}
      document.getElementById('bt').value='';
      ['b1label','b1url','b2label','b2url'].forEach(function(id){var e=document.getElementById(id);if(e)e.value='';});
      var m=document.getElementById('bm');m.textContent='✅ Yuborildi! Maydon tozalandi.';m.style.display='block';setTimeout(function(){m.style.display='none';},2800);}
    else alert('Xato');
  }else{
...
var MATERIALS={};
var MAT_SECS={vocab:'sc-vocab',formula:'sc-formulas',audio:'sc-audio',shadowing:'sc-shadowing',grammar:'sc-grammar',writing:'sc-writing'};
...
def admin_user_access():
    ...
    try:
        conn = get_conn(); cur = conn.cursor()
        cur.execute("UPDATE users SET curator_name=%s, lesson_days=%s, lesson_time=%s WHERE user_id=%s",
                    ((d.get("curator_name") or "").strip()[:60], (d.get("lesson_days") or "").strip()[:20],
                     (d.get("lesson_time") or "").strip()[:10], uid))
        conn.commit(); cur.close(); conn.close()
    except Exception as e:
        print("schedule save:", e)
...
@flask_app.route("/admin/jurnal-students")
def admin_jurnal_students():
    check_api_auth()
    conn = get_conn(); cur = conn.cursor(cursor_factory=RealDictCursor)
    cur.execute("""SELECT u.user_id, u.first_name, u.user_label,
        (SELECT MAX(day) FROM user_completions uc WHERE uc.user_id=u.user_id) AS app_day,
        (SELECT COUNT(*) FROM lessons l WHERE l.user_id=u.user_id) AS lc
        FROM users u
        WHERE u.user_label='kurator' OR EXISTS(SELECT 1 FROM lessons l2 WHERE l2.user_id=u.user_id)
        ORDER BY u.first_name NULLS LAST""")
    rows = cur.fetchall(); cur.close(); conn.close()
    out = [{"user_id": r["user_id"], "name": r["first_name"] or ("ID " + str(r["user_id"])),
            "app_day": int(r["app_day"] or 0), "count": int(r["lc"] or 0)} for r in rows]
    return {"ok": True, "students": out}

@flask_app.route("/admin/jurnal-lessons")
def admin_jurnal_lessons():
    check_api_auth()
    uid = request.args.get("uid")
    if not uid:
        return {"ok": False}, 400
    conn = get_conn(); cur = conn.cursor(cursor_factory=RealDictCursor)
    cur.execute("SELECT id, lesson_date, status, topic, note FROM lessons WHERE user_id=%s ORDER BY lesson_date DESC NULLS LAST, id DESC LIMIT 100", (uid,))
    rows = cur.fetchall(); cur.close(); conn.close()
    out = [{"id": r["id"], "date": str(r["lesson_date"] or "")[:10], "status": r["status"] or "done",
            "topic": r["topic"] or "", "note": r["note"] or ""} for r in rows]
    return {"ok": True, "lessons": out}

@flask_app.route("/admin/lesson-add", methods=["POST"])
def admin_lesson_add():
    ...
    try:
        ...
        conn = get_conn(); cur = conn.cursor()
        cur.execute("INSERT INTO lessons (user_id, lesson_date, status, topic, note) VALUES (%s,%s,%s,%s,%s)",
                    (uid, date, status, topic, note))
        conn.commit(); cur.close(); conn.close()
        return {"ok": True}
    except Exception as e:
        return {"ok": False, "error": str(e)}, 500

@flask_app.route("/admin/lesson-del", methods=["POST"])
def admin_lesson_del():
    check_api_auth()
    try:
        d = request.json or {}
        lid = int(d.get("id"))
...
def admin_broadcast():
    ...
    threading.Thread(target=send_broadcast_sync, args=(text, btns, audience)).start()
    return {"ok": True}
...
def send_broadcast_sync(text, btns=None, audience=None):
    if not bot_app: return
    from telegram.error import Forbidden, RetryAfter
    users = get_broadcast_users(audience)
...
if __name__ == "__main__":
    ...
    scheduler.add_job(send_lesson_reminders, "cron", hour=9, minute=0, id="lesson_reminders", replace_existing=True)
