...
AI_SYSTEM = (
    ...
    "writing.task — darsga mos qisqa yozma topshiriq (o'zbekcha). "
    "writing.sample — shu topshiriqqa darajaga mos namunaviy javob (rus tilida). "
...
def send_welcome(uid, first_name):
    try:
        ...
        text = (f"Salom, {first_name or ''}! \U0001F44B\n\n*Zamira Russian* — ruscha gapirish kursi.\n\n"
                f"Kursga kirish uchun o'qituvchiga murojaat qiling va shu ID ni yuboring:\n\U0001F194 `{uid}`")
        kb = {"inline_keyboard": [[{"text": "\U0001F4DA Darsni boshlash", "web_app": {"url": WEBAPP_URL}}]]}
...
def save_content_full(level, day, d):
    ...
    cur.execute("""
        INSERT INTO content (level, day, title, shadowing_ru, shadowing_uz, razgovor_start,
            ...
            grammar_video, writing_task, writing_sample)
        VALUES (%s,%s,%s,%s,%s,%s,%s::jsonb,%s::jsonb,%s::jsonb,%s::jsonb,%s::jsonb,%s::jsonb,%s::jsonb,%s,%s,%s)
...
def api_content():
    ...
    if not session.get("admin"):
        ...
        if src != day and not (d.get("vocab") or []):
...
def admin():
    ...
    rows_files = "".join(
        "<div class='lrow2 frow' data-fl='%s' style='padding:9px 12px'><div style='flex:1'><b>%s · %s-kun</b> <span class='muted' style='font-weight:400'>%s</span></div>"
        "<a class='btn-sm' href='%s' target='_blank'>Ko'rish</a>"
        "<button class='btn-sm' style='border-color:#e4a6a6;color:#c0392b;margin-left:6px' onclick=\"delFile('%s',%s)\">🗑</button></div>"
        % (esc(x['level']), esc(x['level']), x['day'], esc(x.get('filename') or ''),
    ...
    def _tile(c):
        ...
        return ("<div class='ltile'><div class='ltnum'>%d</div>"
                "<div class='ltt'>%s</div><div class='ltm'>%s</div>"
                "<a class='ltedit' href='/admin/lesson?level=%s&day=%d'>Tahrirlash</a></div>"
                % (c['day'], title, _mini(c), c['level'], c['day']))
    _by = {"A1": [], "B1": []}
    for c in lessons:
...
function fileTab(lvl){
  document.querySelectorAll('.ftab').forEach(function(b){b.style.background='';b.style.color='';});
  var act=document.getElementById('ftab-'+lvl);if(act){act.style.background='#1D9E75';act.style.color='#fff';}
...
<div class="screen" id="sc-writing">
  ...
  <div class="fixed">
    <label class="flbl">Topshiriq (o'zbekcha — o'quvchi nima yozishi kerak)</label>
    <textarea id="f_writing_task" placeholder="masalan: O'zingiz haqingizda 5 ta gap yozing..."></textarea>
    <label class="flbl" style="margin-top:10px;">Namuna javob (rus tilida — o'quvchi tugmani bossa ko'rinadi)</label>
    <textarea id="f_writing_sample" placeholder="Меня зовут... Я живу в..."></textarea>
  </div>
  <button class="save" onclick="saveLesson(1)">💾 Saqlash</button>
</div>
...
async function load(){
  ...
  var w=d.writing||{};document.getElementById('f_writing_task').value=d.writing_task||w.task||'';
  document.getElementById('f_writing_sample').value=d.writing_sample||w.sample||'';
...
async function saveLesson(toMenu){
  ...
  var body={level:LEVEL,day:DAY,
    ...
    writing_task:document.getElementById('f_writing_task').value,
    writing_sample:document.getElementById('f_writing_sample').value,
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Kursga kirish uchun o'qituvchiga murojaat qiling va shu raqamni yuboring:\n"
        f"🆔 `{user.id}`",
