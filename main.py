...
# Student Journal (alohida Railway loyihasi) bilan bog'lash uchun
JOURNAL_URL = os.environ.get("JOURNAL_URL", "").rstrip("/")
BOT_LINK_SECRET = os.environ.get("BOT_LINK_SECRET", "")
...
def get_r2_client():
    ...
    if _r2_client is None:
        ...
        _r2_client = boto3.client(
            ...
            region_name="auto",
            # DIQQAT: aniq timeout yo'q edi — R2 sekinlashsa so'rov ~60s osilib qolib,
            # Suhbatdosh Bot tomonidan "vaqt tugadi" deb hisoblanib, "dars tayyor emas"
            # degan noto'g'ri xabar chiqishiga sabab bo'lgan. Endi qisqa va qat'iy:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and context.args[0].startswith("link_"):
        ...
        if not JOURNAL_URL or not BOT_LINK_SECRET:
            logger.warning("JOURNAL_URL yoki BOT_LINK_SECRET sozlanmagan")
            await update.message.reply_text("⚠️ Hozircha ulanib bo'lmadi, birozdan keyin qayta urinib ko'ring.")
            return
        try:
            import urllib.request, json as _j
            body = _j.dumps({"token": token, "chat_id": user.id}).encode()
            req = urllib.request.Request(
                JOURNAL_URL + "/api/telegram/link", data=body, method="POST",
                headers={"Content-Type": "application/json", "X-Link-Secret": BOT_LINK_SECRET},
            )
            with urllib.request.urlopen(req, timeout=10) as resp:
                data = _j.loads(resp.read().decode())
            name = data.get("full_name", "")
            await update.message.reply_text(
                f"✅ Siz Student Journal'ga muvaffaqiyatli ulandingiz, {name}!\n\n"
                f"Endi darslaringiz haqida shu yerga eslatma keladi. 📚"
            )
        except Exception as e:
            logger.warning(f"Journal link xatosi: {e}")
            await update.message.reply_text("⚠️ Link eskirgan yoki xatolik yuz berdi. Kuratoringizdan yangi link so'rang.")
        return
